...
public class NamedSqlTest {
    ...
    public void parseTest() {
        ...
        //未指定参数原样输出
        Assert.equals("select * from table where id=@id and name = ? and nickName = ?", namedSql.getSql());
        Assert.equals("张三", namedSql.getParams()[0]);
        Assert.equals("小豆豆", namedSql.getParams()[1]);
    }
    ...
    public void parseTest2() {
        ...
        NamedSql namedSql = new NamedSql(sql, paramMap);
        Assert.equals("select * from table where id=? and name = ? and nickName = ?", namedSql.getSql());
        //指定了null参数的依旧替换，参数值为null
        Assert.isNull(namedSql.getParams()[0]);
        Assert.equals("张三", namedSql.getParams()[1]);
        Assert.equals("小豆豆", namedSql.getParams()[2]);
    }
    ...
    public void parseTest3() {
        ...
        NamedSql namedSql = new NamedSql(sql, paramMap);
        Assert.equals(sql, namedSql.getSql());
    }
    ...
    public void parseInTest() {
        ...
        NamedSql namedSql = new NamedSql(sql, paramMap);
        Assert.equals("select * from user where id in (?,?,?)", namedSql.getSql());
        Assert.equals(1, namedSql.getParams()[0]);
        Assert.equals(2, namedSql.getParams()[1]);
        Assert.equals(3, namedSql.getParams()[2]);
    }
