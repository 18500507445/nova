...
public class TokenizerUtilTest {
	...
	public void hanlpTest() {
		...
		Assert.equals("这 两 个 方法 的 区别 在于 返回 值", resultStr);
	}
	...
	public void ikAnalyzerTest() {
		...
		Result result = engine.parse(text);
		String resultStr = IterUtil.join(result, " ");
		Assert.equals("这两个 方法 的 区别 在于 返回值", resultStr);
	}
	...
	public void jiebaTest() {
		...
		Result result = engine.parse(text);
		String resultStr = IterUtil.join(result, " ");
		Assert.equals("这 两个 方法 的 区别 在于 返回值", resultStr);
	}
	...
	public void mynlpTest() {
		...
		Result result = engine.parse(text);
		String resultStr = IterUtil.join(result, " ");
		Assert.equals("这 两个 方法 的 区别 在于 返回 值", resultStr);
	}
