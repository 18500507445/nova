...
- - - 获取到**_method**的值。
    ...
    - **原生request（post），包装模式requesWrapper(也是原声的request)重写了getMethod方法，返回的是传入的值。**
...
springboot mvc的自动配置类WebMvcAutoConfiguration 
...
**doGet调用FramworkServlet的processRequest ->调用doService 其实现在DispatherServlet 会调用doDispath**

SpringMVC功能分析都从 org.springframework.web.servlet.DispatcherServlet-》doDispatch（）

也就是springMVC的处理器映射器HandlerMapping   (mappedHandler = getHandler(processedRequest);)

- getHandler中会调用this.handlerMappings 其包含5个mappingHandler
- 从这5个mapper中循环获取处理器 HandlerExecutionChain handler = mapping.getHandler(request);
- Object handler = getHandlerInternal(request);   再调用getHandlerInternal(request)
- 先获取我们的请求然后获取锁 再调用lookuoHandlerMethod(请求路径，request)
- 使用我们的请求路径从mappingRegistry中找到我们需要的所有路径（请求方式不同）
- 调用addMathingMappings找到所匹配的match
...
![image-20220609142036401](C:\Users\wzh\AppData\Roaming\Typora\typora-user-images\image-20220609142036401.png)
...
protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
    //HandlerMapping：处理器映射。/xxx->>xxxx
    if (this.handlerMappings != null) {  //请求映射都在handlerMappings
...
public class ParameterTestController {
    ...
    public Map<String,Object> getCar(@PathVariable("id") Integer id,     
                                     ...
                                     @RequestParam("age") Integer age,          //多个值时候 集合
                                     @RequestParam("inters") List<String> inters,
                                     @RequestParam Map<String,String> params,
                                     @CookieValue("_ga") String _ga,                   //cookiea 
...
//        map.put("headers",header);
    ...
    @GetMapping("/success")
    public Map carsSell(@RequestAttribute("msg") String msg){  //RequestAttribute请求转发
       
        return ；
    }
    
    //cookie被禁用   常用来使用矩阵变量的方式进行传递
    //   /boos/1：age=20/2;age=20    1==>路径  ；age=10 -->矩阵变量
    ...
    /**
  		WebMvcAutoConfiguration中configurePathMatch配置路径匹配 ----> UrlPathHelper路径帮助器----->
  		类中的removeSemicolonContent成员变量  
    
    */
    @GetMapping("/cars/{path}")
    public Map carsSell(@MatrixVariable("low") Integer low,
                        @MatrixVariable("brand") List<String> brand,
                        @PathVariable("path") String path){   //真实的访问路径
        Map<String,Object> map = new HashMap<>();
...
![image-20220609210448037](C:\Users\wzh\AppData\Roaming\Typora\typora-user-images\image-20220609210448037.png)
...
中央调度器DispatcherServlet类z（找一个处理器适配器）

​	请求映射完成后调用getHandlerAdapter(mappedHandler.getHandler())
...
![image.png](https://cdn.nlark.com/yuque/0/2020/png/1354552/1603263524227-386da4be-43b1-4b17-a2cc-8cf886346af9.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_22%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
![image.png](https://cdn.nlark.com/yuque/0/2020/png/1354552/1605151090728-f7c60e6f-d0c0-4541-bfa3-8cc805dfd5d6.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_21%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
![image.png](https://cdn.nlark.com/yuque/0/2020/png/1354552/1605151359370-01cd1fbe-628a-4eea-9430-d79a78f59125.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_25%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
![image.png](https://cdn.nlark.com/yuque/0/2020/png/1354552/1605260623995-8b1f7cec-9713-4f94-9cf1-8dbc496bd245.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_18%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
![image.png](https://cdn.nlark.com/yuque/0/2020/png/1354552/1605261062877-0a27cc41-51cb-4018-a9af-4e0338a247cd.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_27%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
- ![img](https://cdn.nlark.com/yuque/0/2020/png/1354552/1606024421363-77083c34-0b0e-4698-bb72-42da351d3944.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_14%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)

- ![img](https://cdn.nlark.com/yuque/0/2020/png/1354552/1606024616835-bc491bf0-c3b1-4ac3-b886-d4ff3c9874ce.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_28%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
![img](https://cdn.nlark.com/yuque/0/2020/png/1354552/1606796395719-eb57ab48-ae44-45e5-8d2e-c4d507aff49a.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_19%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
![img](https://cdn.nlark.com/yuque/0/2020/png/1354552/1606797616337-e73010e9-9cac-496d-a177-64b677af5a3d.png?x-oss-process=image%2Fwatermark%2Ctype_d3F5LW1pY3JvaGVp%2Csize_18%2Ctext_YXRndWlndS5jb20g5bCa56GF6LC3%2Ccolor_FFFFFF%2Cshadow_50%2Ct_80%2Cg_se%2Cx_10%2Cy_10)
...
未来每一个微服务在云上部署以后，我们都需要对其进行监控、追踪、审计、控制等。SpringBoot就抽取了

Actuator场景，使得我们每个微服务快速引用即可获得生产级别的应用监控、审计等功能。
