...
import com.nova.pay.entity.data.YeePayData;
import com.nova.pay.entity.data.YeePayRecordDto;
...
public class YeePayUtil {
     ...
     * 返回：url和token
     */
    public Map<String, String> walletLogin(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            //发起方商户编号（标准商户收付款方案中此参数与商编一致，平台商户收付款方案中此参数为平台商商户编号）
            request.addParam("parentMerchantNo", YeePayConfig.PARENT_MERCHANT_NO);
            //商户编号,易宝支付分配的的商户唯一标识
            request.addParam("merchantNo", YeePayConfig.PARENT_MERCHANT_NO);
            //商户用户ID 用户在商户侧的用户id
            request.addParam("merchantUserNo", data.getUserId());
            ...
            //服务器回调地址 开户完成后通知地址
            request.addParam("notifyUrl", YeePayConfig.WALLET_NOTIFY_URL);
     ...
     * }
     */
    public Map<String, String> tradeOrder(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            request.addParam("parentMerchantNo", YeePayConfig.PARENT_MERCHANT_NO);
            //商户收款请求号。可包含字母、数字、下划线；需保证在商户端不重复。合单收款场景中，此参数为合单收款请求号
            request.addParam("orderId", data.getOrderId());
            request.addParam("merchantNo", YeePayConfig.MERCHANT_NO);
            request.addParam("userNo", data.getUserId());
            ...
            request.addParam("notifyUrl", YeePayConfig.PAY_NOTIFY_URL);
            /**
            ...
            request.addParam("csUrl", YeePayConfig.CS_NOTIFY_URL);
            ...
            businessInfo.put("registId", data.getUserId());
     ...
     */
    @Deprecated
    public Map<String, String> authRealName(String realName, String idCard) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            request.addParam("merchantNo", YeePayConfig.MERCHANT_NO);
            ...
            request.addParam("username", realName);
            request.addParam("idCardNo", idCard);
     ...
     */
    public Map<String, String> microAccess(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            ...
            request.addParam("notifyUrl", YeePayConfig.MICRO_NOTIFY_URL);
     ...
     */
    public String cashier(YeePayData data) {
        StringBuilder url = new StringBuilder("https://cash.yeepay.com/cashier/std");
        StringBuilder param = new StringBuilder();
        param.append("appKey=" + YeePayConfig.APP_KEY);
        param.append("&merchantNo=" + YeePayConfig.MERCHANT_NO);
        ...
        param.append("&userNo=").append(data.getUserId());
        param.append("&userType=USER_ID");
        JSONObject ext = new JSONObject();
        if (6 == data.getPayType()) {
            ext.put("limitPayType", "WALLET_PAY");
            ext.put("walletMemberNo", data.getWalletId());
        } else if (7 == data.getPayType()) {
            ext.put("limitPayType", "YJZF");
        }
        param.append("&ext=").append(ext.toString());
        String sign = getSign(param.toString());
     ...
     * 多次分账的总金额不会超过订单金额
     */
    public Map<String, String> divideApply(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
     ...
     * 完结分账
     */
    public Map<String, String> divideComplete(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            //交易发起方商编。与交易下单传入的保持一致
            request.addParam("parentMerchantNo", YeePayConfig.PARENT_MERCHANT_NO);
     ...
     * 退款接口
     */
    public Map<String, String> refund(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            //交易发起方商编。与交易下单传入的保持一致
            request.addParam("parentMerchantNo", YeePayConfig.PARENT_MERCHANT_NO);
            ...
            YopResponse response = YopRsaClient.upload("/rest/v1.0/trade/refund", request);
            if (null != response && StringUtils.isNotBlank(response.getStringResult())) {
                return JSONObject.parseObject(response.getStringResult(), new TypeReference<Map<String, String>>() {
                });
            }
     ...
     */
    public Map<String, String> settleCard(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            //交易发起方商编。与交易下单传入的保持一致
            request.addParam("parentMerchantNo", YeePayConfig.PARENT_MERCHANT_NO);
     ...
     * 结算记录查询
     */
    public Map<String, String> queryRecords(YeePayData data) {
        try {
            YopRequest request = new YopRequest(YeePayConfig.APP_KEY, YeePayConfig.PRIVATE_KEY);
            //交易发起方商编。与交易下单传入的保持一致
            request.addParam("parentMerchantNo", YeePayConfig.PARENT_MERCHANT_NO);
    ...
    private static String getSign(String param) {
        PrivateKey isvPrivateKey = getPrivateKey(YeePayConfig.PRIVATE_KEY);
        DigitalSignatureDTO digitalSignatureDTO = new DigitalSignatureDTO();
        digitalSignatureDTO.setAppKey(YeePayConfig.APP_KEY);
    ...
    public static void main(String[] args) {
        ...
        YeePayData data = new YeePayData();

        Map<String, String> result;
        String userId = "1519700757182410752";
        ...
        data.setMerchantNo("10088552390");
        data.setStartTime("2022-07-15 21:04:16");
