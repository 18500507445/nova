...
(
    ...
    price       decimal(10, 2) unsigned default 0.00              not null comment '金额',
    total       int unsigned                                      null comment '总数',
    stock       int unsigned                                      null comment '库存',
    status      tinyint unsigned        default 0                 not null comment '售卖状态：0售卖，1停售',
...
(
    ...
    status          tinyint unsigned default 0                 not null comment '订单状态：0默认，1成功，2已发货，3失败，4退款，5过期',
    pay_status      tinyint unsigned                           null comment '支付状态：0默认，1成功，2处理中，3失败，4退款',
...
(
    ...
    pay_way           tinyint unsigned                           null comment '1支付宝 2微信 3苹果 4易宝 5谷歌支付 6快手支付 7华为支付 99兑换 ',
...
',
    ...
    weight            int unsigned     default 1                 not null comment '权重',
    status            tinyint unsigned default 0                 null comment '是否生效  0不生效 1生效  -1 作废',
...
(
    ...
    ali_h5           tinyint unsigned default 0                 not null comment '支付宝h5 0关闭 1显示 备注：支付宝h5和app都走h5支付',
    ali_app          tinyint unsigned default 0                 not null comment '支付宝app 0关闭 1显示',
    ali_applet       tinyint unsigned default 0                 not null comment '支付宝小程序 0关闭 1显示',
    ali_logo_url     varchar(128)                               null comment '支付宝logo',
    wechat_h5        tinyint unsigned default 0                 not null comment '微信h5 0关闭 1显示',
    wechat_app       tinyint unsigned default 0                 not null comment '微信客户端 0关闭 1显示',
    wechat_jsapi     tinyint unsigned default 0                 not null comment '微信内原生、小程序 0关闭 1显示',
    wechat_logo_url  varchar(128)                               null comment '微信logo',
    apple            tinyint unsigned default 0                 not null comment '苹果支付 0关闭 1显示',
    apple_logo_url   varchar(128)                               null comment '苹果logo',
    yee_pay_quick    tinyint unsigned default 0                 not null comment '易宝快捷 0关闭 1显示',
    yee_pay_wallet   tinyint unsigned default 0                 not null comment '易宝钱包 0关闭 1显示',
    yee_pay_logo_url varchar(128)                               null comment '易宝logo',
    google_pay       tinyint unsigned default 0                 not null comment '谷歌支付',
...
(
    ...
    pay_config_id bigint unsigned                            null comment '支付配置表id',
    ...
    trade_status  tinyint unsigned default 0                 not null comment '0支付中，1成功，2失败，3退款， 4处理中，5关单',
    fee           decimal(10, 2)   default 0.00              null comment '金额',
    type          tinyint unsigned                           not null comment '1h5 2小程序 3app 4微信原生jsapi 5沙盒 6钱包 7快捷 8球币兑换 9微信(四方支付) 10支付宝(四方支付) 11扫码(微信、支付宝)',
    pay_way       tinyint unsigned                           not null comment '1支付宝 2微信 3苹果 4yeePay 5谷歌 6快手 7华为支付 99金币兑换',
    ...
    business_code tinyint unsigned default 0                 not null comment '充当开通业务code，例如支付成功通知后，发送mq自动处理',
    remark        varchar(64)                                null comment '备注
1.易宝1级收款商户
2.易宝2级默认收款方式 银行卡、钱包
3.苹果支付验签结果
...
(
    id          bigint unsigned auto_increment comment '主键id'
        primary key,
    name        varchar(64)                                null comment '用户名',
    status      tinyint unsigned default 0                 not null comment '用户状态：0正常，1封号',
    remark      varchar(64)                                null comment '备注',
    operator    varchar(64)                                null comment '操作人',
    update_time datetime         default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '修改时间',
    create_time datetime         default CURRENT_TIMESTAMP not null comment '创建时间'
